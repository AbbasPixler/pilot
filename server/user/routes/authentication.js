...
router.post("/login", async (req, res) => {
  try {
    ...
    const user = await User.findOne(req.body).populate('language').populate("company")
    console.log(user)
...
router.post("/fetch/:id", auth, async(req, res) => {
  try{
    const user = await User.findById(req.params.id).populate("language")
    if(user.length < 1){
      return res.json({
        status: false,
...
router.put("/update/:id", auth, async(req, res) => {
  try{
    ...
    const update = await User.findOne({_id: req.params.id}).populate("language").populate('company')
    const userObject = {
      email: update.email,
...
router.put("/approveUser/:id", auth, async(req, res) => {
  try{
    const user = await User.findById(req.params.id)
    ...
    let message = ""
    if(req.body.user.approved == true){
      message = "User approved successfully!"
    }
    else if(req.body.user.approved == false){
      message = "This user has been DisApproved!"
    }
