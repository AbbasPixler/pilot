...
router.post("/create", auth, async (req, res) => { 
  try {
    ...
    const alreadyExist = await Customer.find({ name: req.body.customer.name})
...
router.put("/createContact/:id", auth, async(req, res) => {
  try{
    ...
    customer['contact'].push(req.body.contact)
...
//------------------create a customer contact-------------------
router.put("/editContact/:id", auth, async(req, res) => {
  try{
...
router.put("/update/:id", auth, async(req, res) => {
  try{
    ...
    const checkName = await Customer.findOne({name: req.body.customer.name})

    if(checkName && (checkName['_id'] != req.params.id) && (checkName['name'] == req.body.customer.name)){
      return res.json({
        status: false,
